...
const UserDetails = ({ id, type, userData, subAccounts }: Props) => {
    ...
    useEffect(() => {
        if (data.user) {
            const fetchDetails = async () => {
                const responase = await getAuthUserDetails();
                if (responase) setAuthUserData(responase);
            }
            fetchDetails();
        }
    ...
    const onChangePermission = async (subAccountId: string, val: boolean, permissionsId: string | undefined) => {
        ...
        const responase = await changeUserPermissions(permissionsId ? permissionsId : v4(),
            data.user.email, subAccountId, val)
        if (type === 'agency') {
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
                        ...
                        <Button disabled={form.formState.isSubmitting}
                            type='submit'
                        >{form.formState.isSubmitting ? <Loading /> : 'Save User Details'}</Button>
                                    ...
                                    {subAccounts?.map((subAccount) => {
                                        ...
                                        return <div key={subAccount.id} className='flex  items-center justify-between rounded-lg border p-4'>
                                            ...
                                            <Switch disabled={loadingPermissions} checked={subAccountPermissionsDetails?.access}
                                                onCheckedChange={(permission) => { onChangePermission(subAccount.id, permission, subAccountPermissionsDetails?.id) }}
                                            />
                                        </div>
                                    })}
                                </div>
