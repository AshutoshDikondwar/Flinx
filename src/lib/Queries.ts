...
//     if (!foundAgencyId) {
//         if (!subaccountId) {
//             throw new Error("You need to at least an agency Id or subaccount Id")
//         }
...
export const getAuthUserDetails = async () => {
  try {
    ...
    console.log("get auth user details", getAuthUserDetails);
  }
}
...
// export const verifyAndAcceptInvitation = async () => {
//   try {
//     console.log('Starting verifyAndAcceptInvitation');
...
export const changeUserPermissions = async (permissionId: string | undefined, userEmail: string, subAccountId: string, permission: boolean) => {
  ...
  try {
    ...
    console.log("could not change permission", err);
  }
}
...
export const createMedia = async (subaccountId: string, mediaFile: CreateMediaType) => {
  ...
  const response = db.media.create({
    data: {
      link: mediaFile.link,
      name: mediaFile.name,
...
export const getLanesWithTags = async (pipelineId: string) => {
...
export const updateTicketOrder = async (tickets: Ticket[]) => {
  try {
    const updateTrans = tickets.map((ticket) => db.ticket.update({
      where: {
        id: ticket.id
      },
      data: {
    ...
    console.log(err, 'ERROR UPDATE TICKET ORDER');
  }
}
